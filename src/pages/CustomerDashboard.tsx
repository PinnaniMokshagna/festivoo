...
import {
  ...
  MessageSquare, PhoneCall, Send, X, HelpCircle, Headphones, Search,
  ShieldAlert, FileQuestion, LifeBuoy
} from 'lucide-react';
...
export default function CustomerDashboard() {
  ...
  useEffect(() => {
    if (location.search.includes('support=true') || location.hash === '#support') {
      setShowHelpCenter(true);
    }
  }, [location]);
  const [bookings, setBookings] = useState<BookingWithVendor[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'overview' | 'bookings' | 'chats' | 'saved' | 'invoices'>('overview');
  ...
  // Saved Chats Persistence State
  const [savedChats, setSavedChats] = useState<Record<string, { name: string; category: string; image?: string; lastUpdated: string; messages: Array<{ id: string; sender: 'user' | 'vendor'; text: string; time: string }> }>>({});

  // 24x7 PhonePe-Style Help Center Modal State
  ...
  // Load Saved Chats from localStorage for the customer
  useEffect(() => {
    try {
      const storageKey = `festivo_saved_chats_${user?.email || 'default'}`;
      const existing = localStorage.getItem(storageKey);
      if (existing) {
        setSavedChats(JSON.parse(existing));
      } else {
        const initialChats = {
          'Festivo Customer Support': {
            name: 'Festivo Customer Support',
            category: '24x7 Customer Care',
            ...
            messages: [
              {
                ...
                text: 'Welcome to Festivo 24x7 Customer Support! Your support & vendor conversations are automatically saved here in your customer portal.',
        ...
        localStorage.setItem(storageKey, JSON.stringify(initialChats));
      }
    } catch (e) {
  ...
  // Save Chat Thread helper
  const saveChatThread = (vendorName: string, category: string, messages: Array<{ id: string; sender: 'user' | 'vendor'; text: string; time: string }>, image?: string) => {
    const storageKey = `festivo_saved_chats_${user?.email || 'default'}`;
    setSavedChats(prev => {
      const updatedChats = {
        ...prev,
        [vendorName]: {
          name: vendorName,
          category,
          image: image || prev[vendorName]?.image,
          lastUpdated: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          messages
        }
      };
  ...
  // Interactive Live Chat & Call Handlers with LocalStorage Persistence
  const handleOpenChat = (vendorName: string, category: string, image?: string) => {
    ...
    if (savedChats[vendorName] && savedChats[vendorName].messages.length > 0) {
      setChatMessages(savedChats[vendorName].messages);
    } else {
      const initialMsgs = [
        {
          id: '1',
      ...
      saveChatThread(vendorName, category, initialMsgs, image);
    }
  };
  ...
  const handleSendMessage = () => {
    ...
    saveChatThread(activeChatVendor.name, activeChatVendor.category, updatedWithUser, activeChatVendor.image);
    ...
    setTimeout(() => {
      const vendorReply = {
        ...
        sender: 'vendor' as const,
        text: `Thank you for your message! ${activeChatVendor.name} support has noted your request ("${currentText}"). Our team will confirm your details shortly.`,
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      };
      setChatMessages(prev => {
        const fullMsgs = [...prev, vendorReply];
        saveChatThread(activeChatVendor.name, activeChatVendor.category, fullMsgs, activeChatVendor.image);
        return fullMsgs;
      });
    }, 800);
  };
  ...
  return (
    <>
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl text-sm font-bold transition-all border border-white/10 hover:border-white/30"
                >
      ...
      {showHelpCenter && (
        <div className="fixed inset-0 z-50 bg-dark-900/60 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden border border-sage-100 animate-scale-up flex flex-col">
            {/* Header Banner */}
            <div className="bg-gradient-to-r from-sage-900 via-sage-800 to-sage-900 p-6 text-white relative">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-gold-500/20 backdrop-blur-md border border-gold-400/30 flex items-center justify-center text-gold-400 shadow-glow">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-sage-200 text-xs font-medium">Instant Support</span>
                    </div>
              ...
              <div className="mt-4 relative">
                ...
                <input
                  type="text"
                  placeholder="Search your issue (e.g. Payment deduction, Refund status, Change date)..."
              ...
              <div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    {
                      id: 'faq-2',
                      q: 'How do I cancel or reschedule my event booking?',
                      a: 'Go to "My Bookings" tab, select the specific vendor booking, and click "Reschedule / Contact Vendor". Rescheduling requests submitted 48 hours prior to the event date carry zero penalty fees.'
                    },
                    {
                      id: 'faq-3',
                      q: 'Is my payment via Razorpay 100% safe?',
                      a: 'Yes! All transactions on Festivo pass through Razorpay 256-bit SSL encryption. We adhere to PCI-DSS compliance standards to keep your credentials fully safe.'
                    },
                    {
                      id: 'faq-4',
                      q: 'How do I get an official Tax Invoice receipt?',
                      a: 'Visit the "Invoices" or "Payments" tab in your Customer Dashboard and tap "Download Receipt". You can save or print your GST tax invoice instantly.'
                    }
                  ]
                    .filter(faq => !helpSearchQuery || faq.q.toLowerCase().includes(helpSearchQuery.toLowerCase()) || faq.a.toLowerCase().includes(helpSearchQuery.toLowerCase()))
                    .map(faq => (
                      <div key={faq.id} className="border border-sage-100 rounded-2xl overflow-hidden">
                        <button
                          onClick={() => setActiveFaqId(activeFaqId === faq.id ? null : faq.id)}
                          className="w-full p-3.5 bg-sage-50/50 hover:bg-sage-50 text-left flex items-center justify-between font-bold text-xs text-sage-900 transition-colors"
                        >
                          <span>{faq.q}</span>
                          <ChevronRight className={`w-4 h-4 text-sage-500 transition-transform ${activeFaqId === faq.id ? 'rotate-90' : ''}`} />
                        </button>
                        {activeFaqId === faq.id && (
                          <div className="p-3.5 bg-white text-xs text-dark-600 leading-relaxed border-t border-sage-100">
                            {faq.a}
                          </div>
                        )}
                      </div>
