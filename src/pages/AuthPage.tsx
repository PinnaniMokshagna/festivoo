...
export default function AuthPage() {
  ...
  const { signIn, signUp, user, profile, sendOtp, verifyOtp, updatePassword } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      ...
      navigate(targetRole === 'vendor' ? '/vendor-dashboard' : '/dashboard');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signin') {
      ...
      setLoading(false);
      if (error) { setError(error); return; }
      navigate(selectedRole === 'vendor' ? '/vendor-dashboard' : '/dashboard');
    }
  };
